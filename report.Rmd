...
if(params$chosen_things$action_type == "Build Network"){
  ...
  plant_trait <- temp_data %>% 
    dplyr::select(plant_sp, plant_native, plant_life_form) %>% unique() %>% 
  ...
  colnames(output_table)  <- c("Plant species", "Pollinator guild", "Number of observations", "Plant native", "Plant life form")
